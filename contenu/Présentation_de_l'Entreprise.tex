...
Cette diversité des rôles impose la mise en place d’un système RBAC, ou Role-Based Access Control \cite{owasp}. Ce système permet d’attribuer des permissions précises selon le rôle de chaque utilisateur \cite{owasp}. Il garantit que chaque acteur accède uniquement aux fonctionnalités qui correspondent à ses responsabilités.
...
Enfin, le projet vise à déployer FinderrLink dans un environnement serveur afin de rendre la plateforme accessible en ligne. Cette phase comprend la configuration du VPS, la mise en place des conteneurs Docker, l'utilisation d'un proxy inverse Nginx, la gestion de la base de données PostgreSQL et la connexion avec le service IA Python \cite{docker, nginx, postgresql}. Le déploiement permet de valider le fonctionnement de l'application dans un environnement proche de la production et de préparer son évolution future.
\section{Conclusion}
